...
describe('taskRun', () => {
  describe('setTaskRun', () => {
    ...
    test('invalid property', () => {
      manager.setTask('task1', task);
      // @ts-expect-error property does not exist
      const taskRunId = manager.setTaskRun('task1', undefined, {invalid: 5000});
      expect(manager.getTaskRunConfig(taskRunId)).toEqual({});
    });
    ...
    test('invalid retryDelay', () => {
      ...
      const taskRunId8 = manager.setTaskRun('task1', undefined, undefined, {
        retryDelay: '5.5.5',
      });
...
describe('ticks', () => {
  beforeEach(() => manager.setManagerConfig({tickInterval: 2}));
